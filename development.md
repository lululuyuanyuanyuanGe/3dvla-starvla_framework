...
1.  **Create Directories:** Set up the new folders in `starVLA/model/modules/`.
2.  **Update `__init__.py` files:** Ensure the new folders and modules are importable and reachable by the registry.
...
1.  **Geometric Encoder (`MapAnything`):**
    *   **File:** `starVLA/model/modules/geometric_encoder_model/MapAnything.py`
    *   **Task:** Wrap the MapAnything model.
    *   **Input:** 2D Images.
    *   **Output:** 3D feature representation (e.g., point cloud embeddings or depth-aware tokens).
...
*   **Step 2:** Implement wrappers (SigLIP first, then MapAnything, then Llava3D).
