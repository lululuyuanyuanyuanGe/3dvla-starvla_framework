...
1. 你必须先完成 Step 0（读取并绑定上下文），否则禁止输出任何算法建议。  
2. 下面这份 Markdown 是“关键上下文”，后续每个结论都要优先基于它。  
3. 如果你的先验知识与该 Markdown 冲突，先采用 Markdown，并明确标注冲突点。  
4. 未完成 Step 0 时，你只能返回阻塞原因，不能继续后续步骤。  

## Step 0（必须先执行，且是第一操作）
请先完整阅读以下文档，并将其作为重要上下文载入：

`/Users/bazinga/code/my-starvla/docs/VLM_framework_algorithm_deep_summary.md`

完成阅读后，先输出以下三部分（完成前不得继续）：

1. `CONTEXT_LOADED`：用 3-5 句话说明你已读取该文档，并概括文档主线。  
2. 你从文档中提取到的 10 条关键事实（必须具体，不能泛化）。  
3. 你仍不确定、需要进一步确认的 5 个问题（按优先级排序）。

如果你无法读取该文档，请立即停止并明确说明阻塞原因，不要继续给泛化建议。

## Step 0.5（上下文绑定，必须执行）
在进入后续分析前，先给出一个“上下文索引表”：

1. 列出你后续会反复引用的 8-12 条核心事实，编号为 `[M1]...[M12]`。  
2. 后续所有核心结论（诊断、改进点、路线图、消融设计）都必须至少引用一个 `[Mx]`。  
3. 如果某个建议无法对应任何 `[Mx]`，必须标注“外部推断”，并说明不确定性。  
